...
describe('TokenHotbar.save', () => {
    ...
    it('it will save macros placed in the right slots', async () => {
        ...
        const macrosToSave: HotbarSlots= { '1': '1' , '42': '2' };

        const settings = new Settings();
        settings.hotbarPage = 5;
        const tokenHotbar = new TokenHotbar(token!.id, [{id: '1'}, {id: '2'}], flags, keyStrategy, settings, new ConsoleLogger(settings));

        // Act
        spyOn(flags, 'set').and.callThrough();
        await expectAsync(tokenHotbar.setTokenMacros({ hotbar: macrosToSave })).toBeResolvedTo(true);
